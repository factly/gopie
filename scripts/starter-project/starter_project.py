...
def upload_files_to_s3() -> list[str]:
    """
    ...
    logging.info(f"Found {len(local_files)} files to upload to S3 bucket '{S3_BUCKET_NAME}'.")

    for file_path in local_files:
        object_name = file_path.name
        logging.info(f"Uploading {object_name} to s3://{S3_BUCKET_NAME}/{object_name}...")
        try:
...
def create_dataset_from_s3(project_id: str, s3_path: str):
    ...
    try:
        response = requests.post(url, headers=headers, data=json.dumps(payload), timeout=300) # Longer timeout for ingestion
...
def main():
    ...
    logging.info(f"Starting dataset ingestion for {len(s3_paths)} files into project {project_id}.")
