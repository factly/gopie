...
def is_result_too_large(result: list[dict]) -> tuple[bool, str]:
    ...
    Returns:
        A tuple where the first element is True if the result is too large (based on record count, JSON size, or column count), and the second element is a string explaining the reason. If the result is acceptable or an error occurs, returns (False, "").
    """
    try:
        if len(result) > 200:
...
def truncate_result_for_llm(result: list[dict] | None) -> list[dict] | None:
    ...
    If the input is `None` or contains 10 or fewer records, it is returned unchanged. For larger results, only the first 10 records are kept, and a note is appended indicating the truncation and availability of the full result.
    
    Returns:
        The truncated result list with an appended note if truncation occurred, or the original result if no truncation was needed.
    """
