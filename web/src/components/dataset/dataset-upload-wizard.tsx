...
export function DatasetUploadWizard({ projectId }: DatasetUploadWizardProps) {
  ...
  const [datasetDescriptionTouched, setDatasetDescriptionTouched] =
    useState<boolean>(false);
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [datasetCustomPromptTouched, setDatasetCustomPromptTouched] =
    useState<boolean>(false);
  const [datasetCustomPrompt, setDatasetCustomPrompt] = useState<string>("");
  ...
  const setDatasetDescription = useUploadStore(
    (state) => state.setDatasetDescription
  );
  ...
  const [selectedDriver, setSelectedDriver] = useState<
    "postgres" | "mysql" | null
  >(null);
  const [isValidationWarningDialogOpen, setIsValidationWarningDialogOpen] =
    useState(false);

  // Ref for unified uploader component to trigger upload
  const unifiedUploaderRef = useRef<UnifiedUploaderRef>(null);
  ...
  const handleAutoGenerateDescriptions = useCallback(
    async (summary: Record<string, unknown>, rows: string[][]) => {
      ...
      try {
        ...
        setDescriptionsError(errorMessage);
      } finally {
  ...
  const handleFileValidated = useCallback(
    (
      ...
      console.log("handleFileValidated called with:", {
        file: file?.name,
        validation,
        response,
      });
      ...
      // Don't auto-navigate to step 2 - wait for column descriptions to be generated
    },
  ...
  const handleCreateDataset = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Failed to parse S3 URL from upload response: ${uploadURL}`
        );
      }
  ...
  const handleClearUpload = () => {
    // Clear Uppy state via ref
    if (unifiedUploaderRef.current) {
      unifiedUploaderRef.current.clearUploader();
    }
  ...
  React.useEffect(() => {
    if (
      ...
      handleAutoGenerateDescriptions(
        summary,
        validationResult.previewData as string[][]
      ).catch(() => {
  ...
  // Auto-navigate to step 2 when column descriptions are successfully generated or failed
  React.useEffect(() => {
    if (
      currentStep === 1 &&
      validationResult?.isValid &&
      (descriptionsGenerated || descriptionsError) &&
      !isGeneratingDescriptions &&
      !hasAutoNavigatedRef.current
    ) {
      // Wait a bit before navigating if descriptions were generated successfully
      const delay = descriptionsGenerated ? 2000 : 1000;
  ...
  // Show loading overlay while DuckDB is initializing
    ...
    return (
      <div className="container max-w-5xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-card border rounded-lg">
          <CenteredLoading text="Initializing for file upload" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container max-w-5xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <StepperContent>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-card border p-6">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Upload files from your computer, import from a URL, or select from Google Drive.
                Supports CSV, JSON, Excel, Parquet, DuckDB, and other data formats.
              </p>
              ...
              {descriptionsGenerated && !isGeneratingDescriptions && (
                <Alert className="mb-4 bg-green-50 dark:bg-green-950/30 border-green-200 dark:border-green-900">
                  ...
                  <AlertTitle className="text-green-800 dark:text-green-200">
                    Column Descriptions Generated!
                  </AlertTitle>
                  <AlertDescription className="text-green-700 dark:text-green-300">
                    Your dataset is ready. Moving to the next step...
                  </AlertDescription>
                </Alert>
              )}
              ...
              {descriptionsError && !isGeneratingDescriptions && (
                <Alert className="mb-4 bg-yellow-50 dark:bg-yellow-950/30 border-yellow-200 dark:border-yellow-900">
                  ...
                  <AlertDescription className="text-yellow-700 dark:text-yellow-300">
                    {descriptionsError}. You can still proceed with the upload.
                  </AlertDescription>
                </Alert>
              )}
              ...
              <UnifiedUploader
                ref={unifiedUploaderRef}
                projectId={projectId}
                onUploadSuccess={handleFileValidated}
                onUploadError={handleUploadError}
                onAutoGenerateDescriptions={handleAutoGenerateDescriptions}
                onValidationSuccess={() => {
                  // Don't auto-navigate here anymore - let the column description generation handle it
                }}
              />
            </div>
            ...
            <div className="bg-card border p-6">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Create a dataset by connecting to your existing database or
                cloud storage.
              </p>
            ...
            <div className="bg-card border p-6">
              ...
              {!validationResult ? (
                <div className="text-center py-8 text-muted-foreground">
                  <p>
                    No validation data available. Please go back and upload a
                    file.
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <Alert className="mb-4 bg-green-50 dark:bg-green-950/30 border-green-200 dark:border-green-900">
                    <CheckCircle2 className="h-4 w-4 text-green-600 dark:text-green-400" />
                    <AlertTitle className="text-green-800 dark:text-green-200">
                      DuckDB {validationResult.format?.toUpperCase()} Validation
                      Successful
                    </AlertTitle>
                    <AlertDescription className="text-green-700 dark:text-green-300">
                      {validationResult.format?.toUpperCase()} file is valid
                      with {validationResult.columnNames?.length || 0} columns:
            ...
            <div className="bg-card border p-6">
              ...
              {isProcessing && (
                <Alert className="mb-4 bg-primary/10 border-primary/20">
                  ...
                  <AlertDescription>
                    Your dataset is being created on the server. This may take a
                    few moments...
                  </AlertDescription>
                </Alert>
              )}
        ...
        <div
          style={{
            display: currentStep === 4 && !createdDataset ? "block" : "none",
          }}
        >
          <div className="space-y-6">
            {/* Top Navigation */}
            ...
            <div className="bg-card border p-6">
              ...
              <p className="text-sm text-muted-foreground mb-6">
                Provide a name and description for your dataset before creating
                it.
              </p>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="dataset-custom-prompt">
                    Custom Prompt (Optional)
                  </Label>
                  <Textarea
                    id="dataset-custom-prompt"
                    placeholder="Enter a custom prompt to guide AI interactions with this dataset..."
                    value={datasetCustomPrompt}
                    onChange={(e) => setDatasetCustomPrompt(e.target.value)}
                    onBlur={() => setDatasetCustomPromptTouched(true)}
                    rows={3}
                  />
                  <p className="text-xs text-muted-foreground">
                    This custom prompt will be used when the AI interacts with
                    this dataset.
                  </p>
                </div>
              </div>
      ...
      <Dialog
        open={isValidationWarningDialogOpen}
        onOpenChange={setIsValidationWarningDialogOpen}
      >
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Some rows in your dataset contain data that doesn&apos;t match the
              expected types.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            <div className="bg-yellow-50 dark:bg-yellow-950/30 border border-yellow-200 dark:border-yellow-900 rounded-lg p-4">
              <div className="text-sm text-yellow-800 dark:text-yellow-200 mb-3">
                <strong>
                  {validationResult?.rejectedRows?.length || 0} row(s)
                </strong>{" "}
                will be excluded from your dataset due to data type mismatches:
              </div>
