...
export const FileValidationUppy = forwardRef<FileValidationUppyRef, FileValidationUppyProps>(function FileValidationUppy({
  ...
  const [detectedFormat, setDetectedFormat] =
    useState<SupportedFileFormat | null>(null);

  // Validation state
  const [uploadError, setUploadError] = useState<string | null>(null);
  const [validationResult, setValidationResult] =
    useState<ValidationResult | null>(null);
  const [actualUploadResponse, setActualUploadResponse] = useState<unknown>(null);
  ...
  useEffect(() => {
    ...
    uppyInstance.on("upload-success", async (file, response) => {
      try {
        ...
        console.log('Upload success response structure:', response);
        setActualUploadResponse(response);
    ...
    try {
      ...
      if (!result.isValid) {
        ...
        if (result.columnNames) {
          ...
          // Call onUploadSuccess callback immediately after validation success
          if (onUploadSuccess && file) {
            // Create a fake UppyFile-like object for the callback
            const fakeUppyFile = {
              name: file.name,
              size: file.size,
              type: file.type,
            ...
            console.log('Calling wizard callback after validation success and column store update');
            // Use the actual upload response instead of a fake one to preserve S3 URL
            const responseToUse = actualUploadResponse || { success: true };
            onUploadSuccess(fakeUppyFile, responseToUse, result);
          }
