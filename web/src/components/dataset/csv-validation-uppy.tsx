...
export const FileValidationUppy = forwardRef<
  FileValidationUppyRef,
  FileValidationUppyProps
>(function FileValidationUppy(
  {
    projectId,
    onUploadSuccess,
    onAutoGenerateDescriptions,
  ...
  const detectedFormat = useUploadStore(
    (state) => state.detectedFormat
  ) as SupportedFileFormat | null;
  ...
  const setOriginalFileName = useUploadStore(
    (state) => state.setOriginalFileName
  );
  ...
  useEffect(() => {
    ...
    const uppyInstance = new Uppy({
      ...
      restrictions: {
        ...
        maxFileSize: 100 * 1000 * 1000 * 1000, // 100GB limit (increased from 10GB)
      },
      autoProceed: false,
  ...
  const handleFileSelected = async (file: File) => {
    ...
    try {
      ...
      // Read only the first 1GB for validation for CSV/JSON to avoid memory issues
      const ONE_GB = 1024 * 1024 * 1024;
      let buffer: ArrayBuffer;
      if (format === "csv" || format === "json") {
        const blobPart = file.size > ONE_GB ? file.slice(0, ONE_GB) : file;
        buffer = await blobPart.arrayBuffer();
      } else {
      ...
      if (!result.isValid) {
        ...
        if (fileSize > 1000 * 1000 * 1000) {
          toast.success(
            `${getFileFormatDisplay(
              format
            )} validation successful! (Large file - first 1GB validated)`
          );
        } else {
          toast.success(
        ...
        if (result.columnNames) {
          console.log("Setting column mappings with:", {
            columnNames: result.columnNames,
            columnTypes: result.columnTypes,
          });
          setColumnMappings(result.columnNames, result.columnTypes);
          console.log(
            "Column mappings set, calling getColumnMappings to verify:",
            getColumnMappings()
          );
    ...
    <div className="w-full">
      ...
      {validationResult?.isValid &&
        ...
        validationResult.rejectedRowCount > 0 && (
          <Alert
            variant="default"
            className="mb-4 border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950"
          >
            ...
            <AlertDescription className="text-amber-800 dark:text-amber-200">
              <div className="space-y-2">
                <p>
                  ...
                  doesn&apos;t match the expected types and will be excluded
                  from the dataset:
                </p>
                  ...
                  validationResult.rejectedRows
                    ...
                    .map((rejection, index) => {
                      ...
                      if (
                        ...
                        rejection.actualValue.trim() !== ""
                      ) {
                        // Show actual value if it exists and is not just whitespace
                        const actualValue =
                          rejection.actualValue.length > 50
                      ...
                      const displayExpectedType =
                        rejection.expectedType === "unknown"
                          ? "a valid type"
                      ...
                      return (
                        <div
                          key={index}
                          className="text-xs bg-amber-100 dark:bg-amber-900 p-2 rounded border border-amber-200 dark:border-amber-700"
                        >
                          <span className="font-medium">
                            Row {rejection.rowNumber}:
                          </span>{" "}
      ...
      <div className="mb-4 text-sm text-gray-600">
        <p>
          <strong>Supported formats:</strong> CSV, Parquet, JSON, Excel (.xlsx,
          .xls), DuckDB (.duckdb)
        </p>
        {detectedFormat && (
          <p>
      ...
      <CustomFileUploader
        ...
        fileInfo={
          validationResult && !selectedFile
            ? {
                name: `Dataset file (${
                  validationResult.format?.toUpperCase() || "Unknown"
                })`,
                size: 0,
