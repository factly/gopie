...
export const useColumnNameStore = create<ColumnNameState>((set, get) => ({
  ...
  updateColumnName: (originalName: string, updatedName: string) => {
    set((state) => {
      const currentMapping = state.columnMappings[originalName];
  ...
  autoFixAllColumns: () => {
    set((state) => {
      const newMappings: Record<string, ColumnNameMapping> = {};

      // Process all column mappings
      Object.entries(state.columnMappings).forEach(([key, mapping]) => {
