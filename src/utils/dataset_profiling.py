...
from src.lib.graph.types import DatasetSchema
...
"""
SQL Constraints Detected:

1. PRIMARY KEY
   - Uniquely identifies each row in a table
   - Cannot contain NULL values
   - Often used in relationships between tables

2. FOREIGN KEY
   - References a PRIMARY KEY in another table
   - Creates relationships between tables
   - Often indicated by naming patterns (_id, etc.)
...
4. NOT NULL
   ...
   - Critical for required data fields

5. CHECK
   - Value range limitations (positive)
   - Data validation constraint

6. TYPE
   - Specialized data type constraint:
     - integer: Values are whole numbers
     - boolean: Binary/boolean values (0/1)
     - year: Values represent years
...
7. DEFAULT
   - Common value for the column
   - Used when inserting new records

8. Column Relationships
   - one-to-one: Both columns contain unique values
   - one-to-many: First column has unique values, second has duplicates
   - many-to-one: First column has duplicates, second has unique values
   - many-to-many: Both columns contain duplicate values

Table JOIN relationships are identified based on:
1. Foreign keys referencing primary keys
2. Columns with the same name across tables
3. SQL JOIN syntax is suggested for each relationship
"""
