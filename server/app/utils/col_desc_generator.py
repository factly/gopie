...
def generate_column_descriptions(dataset_schema: DatasetSchema) -> dict:
    """
    Generate accurate column desc using an LLM based on column metadata.

    Args:
        dataset_schema: DatasetSchema object containing dataset metadata and
        column information
    ...
    try:
        ...
        for column in dataset_schema["columns_details"]:
            col_data = {
                "name": column["name"],
                "type": column["type"],
                "sample_values": _format_sample_values(
        ...
        prompt_template = """
            You are a data expert specialized in understanding dataset schemas.
            Your task is to generate accurate, logical descriptions for
            database columns that explain what each column represents in
            business terms.

            Dataset name: {dataset_name}
            Dataset has {row_count} rows and {col_count} columns.

            For each column below, analyze its name, data type, and sample
            values to infer what information it contains. Provide a concise
            but informative description (1-2 sentences) that would help the
            llm understand the column's purpose.

            Be specific about what the column represents. For ID columns,
            explain what entity they identify. For date columns, explain what
            event or timing they track. For numeric columns, explain what
            quantity they measure.

            Column information:
            {column_info}

            Return ONLY a valid JSON object where keys are the exact column
            names and values are your descriptions.
            Format:
            {{
...
def _generate_fallback_descriptions(
    ...
    for column in dataset_schema["columns_details"]:
        ...
        descriptions[
            col_name
        ] = f"{col_name.replace('_', ' ').title()} ({col_type})"
